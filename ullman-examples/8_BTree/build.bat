...
echo     %__BEG_O%compile%__END%     generate program image
...
:run
if exist "%_IMAGE_FILE%" ( call :run_image
) else ( call :run_script
)
goto :eof
...
) else if %_VERBOSE%==1 ( echo Execute SML script "!__SCRIPT_FILE:%_ROOT_DIR%=!" ^(args: %_MAIN_ARGS%^) 1>&2
)
call "%_SML_CMD%" "%__SCRIPT_FILE%" %_MAIN_ARGS% 2>NUL
if not %ERRORLEVEL%==0 (
    set CM_VERBOSE=%__CM_VERBOSE%
    echo %_ERROR_LABEL% Failed to execute SML script "!__SCRIPT_FILE:%_ROOT_DIR%=!" ^(args: %_MAIN_ARGS%^) 1>&2
    set _EXITCODE=1
    goto :eof
)
set CM_VERBOSE=%__CM_VERBOSE%
...
) else if %_VERBOSE%==1 ( echo Execute SML image "!_IMAGE_FILE:%_ROOT_DIR%=!" ^(args: %_MAIN_ARGS%^) 1>&2
)
call "%_SML_CMD%" @SMLload "%_IMAGE_FILE%" %_MAIN_ARGS%
if not %ERRORLEVEL%==0 (
    echo %_ERROR_LABEL% Failed to execute SML image "!_IMAGE_FILE:%_ROOT_DIR%=!" ^(args: %_MAIN_ARGS%^) 1>&2
    set _EXITCODE=1
    goto :eof
)
goto :eof
