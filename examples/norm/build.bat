...
set "_EXE_FILE=%_TARGET_DIR%\%_PROJ_NAME%.exe"
...
@rem https://github.com/kfl/mosml/blob/master/man/mosmlc.1
set __MOSMLC_OPTS=-toplevel -o "%_EXE_FILE%" -P sml90
...
:run
if exist "%_EXE_FILE%" ( call :run_native
...
) else if %_VERBOSE%==1 ( echo Execute SML program "!_EXE_FILE:%_ROOT_DIR%=!" %_MAIN_ARGS% 1>&2
)
call "%_EXE_FILE%" %_MAIN_ARGS%
if not %ERRORLEVEL%==0 (
    echo %_ERROR_LABEL% Failed to execute SML program "!_EXE_FILE:%_ROOT_DIR%=!" 1>&2
    set _EXITCODE=1
    goto :eof
)
goto :eof
