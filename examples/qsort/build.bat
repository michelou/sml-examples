...
echo load "CommandLine"; load "Int"; load "Math"; load "OS"; > "%__SCRIPT_FILE%"
echo. >> "%__SCRIPT_FILE%"
...
set "__SCRIPT_FILE=%_TARGET_DIR%\%_MAIN_NAME%.sml"
(
    echo ^(* Always include these at the top of your SML files so 
    echo  large datatype values and lists will print fully *^)
    echo Control.Print.printDepth := 100;
    echo Control.Print.printLength := 100;
    echo.
) >> "%__SCRIPT_FILE%"
type "%_SOURCE_DIR%\%_MAIN_NAME%.sml" >> "%__SCRIPT_FILE%"
echo. >> "%__SCRIPT_FILE%"
