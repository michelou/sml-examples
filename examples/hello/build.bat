...
set _MLTON_CMD=
if exist "%MLTON_HOME%\bin\mlton.bat" (
    set "_MLTON_CMD=%MLTON_HOME%\bin\mlton.bat"
)
...
if "%__ARG:~0,1%"=="-" (
    ...
    ) else if "%__ARG%"=="-mlton" ( set _TOOLSET=mlton
...
if %_TOOLSET%==mlton if not defined _MLTON_CMD (
    echo %_WARNING_LABEL% MLton installation directory not found 1>&2
    set _TOOLSET=smlnj
)
...
echo     %__BEG_O%-mlton%__END%      select MLton toolset
...
if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_MLTON_CMD%" %__MLTON_OPTS% "%_SOURCE_DIR%\%_MAIN_NAME%.sml" 1>&2
...
) else if %_TOOLSET%==mlton ( call :run_mlton
...
echo load "CommandLine"; load "Math"; load "OS"; > "%__SCRIPT_FILE%"
echo. >> "%__SCRIPT_FILE%"
